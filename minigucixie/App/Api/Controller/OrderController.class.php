...
class OrderController extends PublicController {
    ...
    public function sendContent(){
    	...
    	$pid = intval($_REQUEST['pro_id']);
    	$tmp = M('product_dp')->where('uid='.$uid.' AND pid='.$pid.' AND order_id='.$order_id)->select();
    	if($tmp){
    		echo json_encode(array('status'=>0,'err'=>'已对该商品评价过'));
			exit();
    	}
    	...
    	$data['addtime'] = time();
    	$res = M('product_dp')->add($data);
    	if($res){
            echo json_encode(array('status'=>1,'err'=>'评价成功！'));
			exit();
        }else{
        	echo json_encode(array('status'=>0,'err'=>'评价失败！'));
			exit();
        }
    }
