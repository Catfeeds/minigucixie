...
class ProductController extends PublicController{
	...
	public function add(){	
		...
		try{	
			...
			if(intval($id)>0){
				...
				if ($adv_str!='') {
					if ($imgs!='') {
						$array['photo_string'] = $imgs.$adv_str;
					}else{
	...
	*/
	...
	//说明：产品 设置推荐
	//***************************
	public function set_hottj(){
		$pro_id = intval($_REQUEST['pro_id']);
		$tj_update=M('product')->field('shop_id,is_hottype')->where('id='.intval($pro_id).' AND del=0')->find();
		if (!$tj_update) {
			$this->error('产品不存在或已下架删除！');
			exit();
		}
		...
		//dump($tj_update);
		$data = array();
		$data['is_hottype'] = $tj_update['is_hottype']==1 ? 0 : 1;
		$up = M('product')->where('id='.intval($pro_id))->save($data);
		if ($up) {
			$this->redirect('index',array('page'=>intval($_REQUEST['page'])));
			exit();
    ...
    //**********************************************************
	//说明：商品属性规格组合 设置单产品不同规格不同价格不同库存
	//**********************************************************
	public function pro_attr(){
		$pro_id = intval($_REQUEST['pid']);
		$check_info = M('product')->where('id='.intval($pro_id))->field('id,name,price_yh,num,company,pro_buff')->find();
		if (!$check_info) {
			$this->error('产品信息错误！');
			exit();
		}

		$checkid = M('pro_attr')->where('pid='.intval($pro_id))->limit(1)->getField('id');
		$check_guige = M('guige')->where('pid='.intval($pro_id))->select();
		if (!intval($checkid) && $check_info['is_buff']>0 && $check_guige) {
			$this->set_pro_attr($pro_id);
		}

		//获取已有属性规格组合
		$attr_list = M('pro_attr')->where('pid='.intval($pro_id))->select();
		foreach ($attr_list as $k => $v) {
			$arr = explode(',', $v['guige_arr']);
			$attr_list[$k]['guige_name'] = '';
			foreach ($arr as $key => $val) {
				$guige_name = M('guige')->where('id='.intval($val))->getField('name');
				if ($key==0) {
					$attr_list[$k]['guige_name'] .= $guige_name;
				}else{
					$attr_list[$k]['guige_name'] .= ' | '.$guige_name;
				}
			}
		...
		if ($check_info['pro_buff']!='') {
			...
			foreach ($pro_buff as $k => $v) {
				...
				$buff['is_img'] = M('attribute')->where('id='.intval($v))->getField('is_img');
	...
	public function set_pro_attr($pro_id){
		...
		//获取产品所有规格属性组合，没有就添加
		$proAttrlist = M('pro_attr')->where('pid='.intval($pro_id))->select();
		//遍历查询到的属性名称
		$d = array();$pro_buff=array();
		$buff = M('product')->where('id='.intval($pro_id))->getField('pro_buff');
		$pro_buff = explode(',', $buff);
		foreach ($pro_buff as $k => $v) {
			$a = M('guige')->where('pid='.intval($pro_id).' AND attr_id='.intval($v))->field('id')->select();
