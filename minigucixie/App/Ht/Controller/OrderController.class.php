...
class OrderController extends PublicController{
	...
	*/
	...
	public function content(){
		//分页
		$count   = M('product_dp')->count();// 查询满足要求的总记录数
		$Page    = new \Think\Page($count,25);// 实例化分页类 传入总记录数和每页显示的记录数(25)
	    ...
	    */
		...
		$show    = $Page->show();// 分页显示输出
		// 进行分页数据查询 注意limit方法的参数要使用Page类的属性
		$content = M('product_dp')->limit($Page->firstRow.','.$Page->listRows)->select();
		if($content){
			foreach($content as $k => $v){
				$content[$k]['username'] = M('user')->where('id='.intval($v['uid']))->getField('uname');
				$content[$k]['proname'] = M('product')->where('id='.intval($v['pid']))->getField('name');
			}
		}
	...
	public function reply_content(){
		...
		if($_POST['submit']==true){
			...
			$res = M('product_dp')->where('id='.$id)->save($data);
			if($res){
				$this->success('回复成功！','content');
				exit();
			}else{
				$this->error('回复失败！');
			}
		}
		$reply = M('product_dp')->where('id='.$id)->find();
	...
	public function pass_content(){
		$id = intval($_GET['id']);
		$data = array();
		$audit =  M('product_dp')->where('id='.$id)->getField('audit');
		if(intval($audit)==0){
			$data['audit'] = 1;
		}else{
			$data['audit'] = 0;
	...
	public function del_content(){
		$id = intval($_GET['id']);
		$res = M('product_dp')->where('id='.$id)->delete();
		if($res){
			$this->success('操作成功！');
			exit();
		}else{
